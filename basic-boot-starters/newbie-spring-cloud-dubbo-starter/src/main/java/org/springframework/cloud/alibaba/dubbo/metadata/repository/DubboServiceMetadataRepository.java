...
public class DubboServiceMetadataRepository {
    ...
    /**
     * The prefix of {@link DubboMetadataService} : "services.aop-service."
     */
    public static final String DUBBO_METADATA_SERVICE_PREFIX = "services.aop-service.";

    /**
     * The {@link URL URLs} property name of {@link DubboMetadataService} : "services.aop-service.urls"
     */
    public static final String DUBBO_METADATA_SERVICE_URLS_PROPERTY_NAME = DUBBO_METADATA_SERVICE_PREFIX + "urls";

    /**
     * The {@link String#format(String, Object...) pattern} of services protocols port
     */
    public static final String DUBBO_PROTOCOLS_PORT_PROPERTY_NAME_PATTERN = "services.protocols.%s.port";
...
//    }
    ...
    public void initialize(String serviceName) {
        ...
        if (logger.isInfoEnabled()) {
            logger.info("The REST aop in the services services has been loaded in the Spring application[name : {}]", serviceName);
        }
    }
        ...
        if (ALL_DUBBO_SERVICES.equals(dubboCloudProperties.getSubscribedServices())) {
            ...
            if (logger.isWarnEnabled()) {
                logger.warn("Current application will subscribe all services(size:{}) in registry, " +
                        ...
                        subscribedServices.size(), "services.cloud.subscribed-services");
            }
        } else {
