package com.newbie.core.util.io;

import com.google.common.base.Predicate;
import com.google.common.graph.Traverser;
import com.google.common.io.Files;
import com.newbie.core.util.text.WildcardMatcher;
...
public class FileTreeWalker {
	...
	public static List<File> listAll(File rootDir) {
		return StreamSupport.stream(Files.fileTraverser().depthFirstPreOrder(rootDir).spliterator(),false).collect(Collectors.toList());
	}
	...
	public static List<File> listFile(File rootDir) {
		return StreamSupport.stream(Files.fileTraverser().depthFirstPreOrder(rootDir).spliterator(),false).filter(x->x.isFile()).collect(Collectors.toList());
	}
	...
	public static List<File> listFileWithExtension(final File rootDir, final String extension) {
		return StreamSupport.stream(Files.fileTraverser().depthFirstPreOrder(rootDir).spliterator(),false).filter(new FileExtensionFilter(extension)).collect(Collectors.toList());
	}
	...
	public static List<File> listFileWithWildcardFileName(final File rootDir, final String fileNamePattern) {
		return StreamSupport.stream(Files.fileTraverser().depthFirstPreOrder(rootDir).spliterator(),false).filter(new WildcardFileNameFilter(fileNamePattern)).collect(Collectors.toList());
	}
	...
	public static List<File> listFileWithRegexFileName(final File rootDir, final String regexFileNamePattern) {
		return StreamSupport.stream(Files.fileTraverser().depthFirstPreOrder(rootDir).spliterator(),false).filter(new RegexFileNameFilter(regexFileNamePattern)).collect(Collectors.toList());
	}
	...
	public static List<File> listFileWithAntPath(final File rootDir, final String antPathPattern) {
		return StreamSupport.stream(Files.fileTraverser().depthFirstPreOrder(rootDir).spliterator(),false)
				.filter(new AntPathFilter(FilePathUtil.concat(rootDir.getAbsolutePath(), antPathPattern))).collect(Collectors.toList());
	}
